...
fprintf(['This file illustrates the use of SSBBO by showing how ',...
...
solverPath = input(['Insert the SSBBO path \n',...
    '>> solverPath='],'s');
...
if fullinfo    % define stop and print criteria
    ...
    st = []; % budgets are chosen inside SSBBO
end
...
if Tuning % self-tuning and info
   ...
   tune = []; % full tuning inside SSBBO is used
end
   

 % call SSBBO 
[x,f,info] = SSBBO(fun,x,st,tune);
...
if ~isempty(info.error),
   error = info.error
    ...
    % display output
    if st.prt>=0, 
        disp('SSBBO completed silently'); 
        disp(' ');
        disp('display output');
        disp(' ');
        info               % progress report by SSBBO
